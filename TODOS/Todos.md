### Open and big tasks 

**PhD** 
- [ ] Background for each topic 
	- [ ] tackle each file group and Mendeley
	- [ ] Write the relevant to least relevant as para and maintain a excel sheet list 
	- [ ] manage the website links and notes on it 
	- [ ] Manage the regulation - current regulation and regulatory structure 
- [ ] Thesis - Monograph 
	- [ ] Start a Latex file and have a gitlab backup - monthly branches 
	- [ ] What wouldn't be possible 10 years ago what wouldn't be 
	- [ ] fix the chapters and write about each part  with small note or intro 
	- [ ] After reading the papers -  write the outline of research hypothesis and research question 
	- [ ] How my results look like 
	- [ ] Grid / map of my result will look like
- [ ] Research visit 
	- [ ] What data I take 
	- [ ] Key details of the Proposal 
	- [ ] Simulation setup - Share Gitlab to Enrique 
	- [ ] Data collection 
	- [ ] Simulation part 
	- [ ] What are the open questions that will be worked there 
	- [ ] ROS2 and preparation to meet Lakeside Labs
- [ ] PhD talk  - Dec 4th
	- [ ] Mesh hardware with Pixhawk 
	- [ ] Research visit topic
	- [ ] Network coding - SoA and next steps - Research gap
- [ ] Hardware setup and data collection 
	- [ ] Mesh hardware 
		- [ ] Goal of Air-to-Air algorithm 
		- [ ] IEEE 802.11p comparison 
		- [ ] first round test setup 
		- [ ] Second test setup with Rasp pi and pixhawk 
		- [ ] Third test setup directly with pixhawk 
	- [ ] 5G dongle 
		- [ ] Fix the SIM Card issue 
		- [ ] Rasp pi to dongle setup 
		- [ ] Pixhawk - rasp pi to dongle 
	- [ ] 5G and Satcom terminal 
		- [ ] Try using 5G terminal 
		- [ ] Mention quotation for satcom usage 
		- [ ] Plan the test from April 
- [ ] Algorithm 
	- [ ] Justify the boundaries of Network coding
	- [ ] Why we use network coding and how far we go 
	- [ ] Test setup scenarios
		
	
	























16/8/25
- [ ] HW purchase list 
- [ ] Topic discussion with Roshan 
- [ ] WCNC - NTN paper 
- [ ] SESAR - Data mesh paper 
- [ ] IEEE survey paper 
- [ ] Email Topic working - Bettstetter
- [x] paper review
- [ ] Tasks with AirMetro - plan time to do 
- [ ] Multi-Link simulator setup
- [ ] Literature 
- [ ] Email GSMA


Personal 
- [ ] Backup on harddisk 
- [ ] Setup NAS / Cloud storage - dropbox 
- [ ] Learn project management and goals - PhD 

---
19/8 
- [x] Paper review
- [x] NTN meeting 
- [x] Section III 
	- [ ] WPs 
	- [x] Milestone
	- [x] Risks
	- [x] Resources
- [x] Section II
	- [ ] Impacts 
	- [ ] Measures 
- [x] Section I 
	- [x] Methods
	- [x] Objective
	- [x] Ethics
- [x] Finalize student meeting for tomorrow
- [x] Write the table and send to Bianca
- [ ] Email to Hamburg aviation contacts 
- [x] NPC document email to Claudia
- [ ] GSMA email 
- [ ] Things to edit in the proposal

---
Things to add / edit in proposal 
Sunday---
- [x] Abbreviation 
- [x] References
- [x] Executive summary
- [x] Images
- [x] tables
Sat ---
- [x] Objective and ambition section - redefine - move technical objective section into the methodology 
- [x] Methodology - go into details 
- [x] Pilot action
Friday afternoon----
- [x] Ethics -DNHS
- [x] AI - Check with guillemette 
- [x] Gender dimension - Check with Vaishnavi
- [x] data and resource management 
Saturday---
- [x] Project Pathway 
- [x] EO- EI
- [x] Scale and significance
- [x] Potential Barrier
Sunday---
- [x] Measure to Maximize 
- [x] Dissemination
- [x] Communication
- [x] Exploitation
- [x] WPs and description

---

22/8 to dos: 
- [ ] GSMA email 
- [ ] ACJA
- [ ] AAM Institute
- [ ] 
- [x] Hamburg aviation email 1
- [x] Hamburg aviation email 2
- [x] Ethics 
- [x] Ai
- [x] Data management 
- [x] Gender - tasks
- [x] Home to dos

---
Impact section 
1. What all should be in project pathway
	- Direct results 
	- benifit the target group by D, C actions 
	- Shrot long term are imapct and outcomes 
	- Proejet centered  around whom? 
	- Direct and indirect results 
	- 
2. EI
3. EO
4. Scale scignificance
5. potential barrier
Summary table 
what are specific needs that triggered this project 
expected results 
what are the dissmenication, Explotatin resulst apply to this resuzlts
Who will use or further up-take the results of the project? Who will benefit from the results of the project?
What change do you expect to see after successful dissemination and exploitation of project results to the target group(s)?
Expected wider scientific, economic and societal effects of the project contributing to the expected impacts in the destination?
_who_ benefits, _what_ changes, and _by when_.


---
26/8 
- [ ] Kpi Table 
- [ ] WP description 
- [x] Resources 
- [ ] Part A
- [ ] NTN meeting
- [ ] Claudia Meeting 
- [ ] Pathway
- [ ] EO
- [ ] EI
- [ ]  Relevant projects email 
	- [ ] Email to GSMA
	- [ ] ACJA
	- [x] AAM Institute
	- [x] AirMouR (Project Manager)
	- [x] code Zero
	- [x] Unchain
	- [ ] Disco
	- [ ] Assured UAM
	- [ ] 2 email POLIS - Pedro Homemde Gouveia,  Manon Coyne
	- [ ] https://www.dronea.lt/en/our-team
	- [ ] https://mobilityalliance.eu/contact/ (Mostly No)
	- [ ] GreenTurn
	- [x] RECIPROCITY
	- [ ] i-LUM
	- [ ] IAM-OSA
	- [ ] 
---
Task for next three hours 
- [x] Part B Pestle 
- [x] Scale
- [x] Summary 
- [x] WP description Bullet points
- [x] Email Morten
- [x] Email Jop
- [x] Email Jiajing
- [x] Email all
- [x] Email Thorsten

----
Goal towards 13th November 2025
Hardware part 
- [ ] Setup Mesh nodes 
- [ ] Pixhawk - Rasp pi to mesh nodes 
- [ ] Fix Simcard issue with Sebastian 
- [ ] Work on the LTE dongle 
- [ ] Raspberry pi with LTE dongle 

Logical and Simulation part 
- [ ] Setup VM  simulator with Satcom and Cell emulator 
- [ ] Network coding basics, and coding for Sat + Cell network - algorithm , goal and implementation 

13-14th Workshop 
- [ ] Presentation planning and ideas
- [ ] Preparation 
- [ ] Trial talk in team meeting 

Other open tasks
- [x] Research proposal
- [x] Meeting with Enrique
- [x] Frank - PhD admission 
- [x] Atmosphere description email
- [x] Discussion continue with Tung
- [x] Discussion with Roshan on paper 
- [ ] Dronnect paper 
- [x] backup 
- [ ] Clean links and storage 

---

